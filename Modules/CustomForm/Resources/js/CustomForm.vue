        ...
        <form :action="form.action" :method="form.method" @submit.prevent="onSubmit"
            ...
            <div class="form-row">
                ...
                <div class="form-group" :class="[field.block_class ? field.block_class : 'col-12']"
                    ...
                    <textarea class="form-control" v-model="field.value" :placeholder="field.placeholder"
                ...
                <div class="form-group" :class="[field.block_class ? field.block_class : 'col-12']"
                    ...
                    <div class="custom-control" :class="'custom-' + field.type" v-for="option in field.options">
                        <input :type="field.type" class="custom-control-input" :name="field.name" v-model="field.value"
                               :id="[option.option_id ? option.option_id : 'key_' + option.id]" :value="option.value">
                ...
                <div class="form-group" :class="[field.block_class ? field.block_class : 'col-12']"
                    ...
                    <select class="form-control"
                            :id="field.field_id ? 'select_' + field.field_id : 'select_' + field.id"
                            v-model="field.value">
            ...
            <div class="text-center my-3">
                <button class="btn-custom" type="submit">
                    <span v-show="!loading">{{ trans.send }}</span>
                </button>
            </div>
        </form>
    </div>
...
<script>
    export default {
        ...
        methods: {
            getForm() {
                axios.get(this.source).then((resource) => {
                    if (typeof resource.data.data !== "undefined")
                        this.form = resource.data.data;
                    this.trans = resource.data.trans
                }).catch((error) => {
                    console.log(error)
                })
            },
