        ...
        <b-list-group>
            <b-list-group-item v-for="(route, index) in routes" :key="index">
                ...
                <b-list-group v-if="typeof route.children !== 'undefined' && route.children.length">
                    <router-link class="list-group-item" :to="{name: child.name}" v-for="(child, key) in route.children" :key="key">
    ...
    export default {
        ...
        methods: {
            ...
            generateTransKey(route) {
                ...
                return route.meta.key_title !== 'dashboard' ? moduleName.replace(' ', '') + '.' + route.meta.key_title : moduleName.toLowerCase();
            }
        }
    }
</script>