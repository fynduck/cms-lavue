...
class SetLocale
{
    ...
    public function handle(Request $request, Closure $next)
    {
            ...
            if (is_null($preferLangSlug) && $acceptLang) {
                ...
                if(count($clientLanguages)) {
                    foreach ($clientLanguages as $clientLangSlug) {
                        $preferLangSlug = Language::where('slug', substr($clientLangSlug, 0, 2))->where('active', 1)->value('slug');
                        if($preferLangSlug)
                            break;
                    }
                }
        ...
        if (!in_array($request->getMethod(), ['POST', 'PUT']))
            return $next($request)->cookie('prefer_lang', $preferLangSlug, now()->addYear()->diffInMinutes(), null, null, config('app.ssl'), false);
