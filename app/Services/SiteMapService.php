...
class SiteMapService
{
    public function generateMap()
    {
        ...
        foreach ($domainLang as $item) {
            $siteMap->add(Url::create(config('app.url') . '/' . $item)
                ->setPriority(1.0));
        }

        PageTrans::where('status', 1)->where('url', '!=', '')->get()->each(function (PageTrans $page) use ($siteMap, $lang) {
            if (count($lang) > 1)
        ...
        Article::leftJoin('article_trans', 'articles.id', '=', 'article_trans.article_id')
            ->where('active', 1)
            ->get()->each(function (Article $newsItem) use ($siteMap, $lang, $pageUrls) {
                if (count($lang) > 1)
                    $addUrl = Url::create(url($lang[$newsItem->lang], [$pageUrls[$newsItem->type][$newsItem->lang], $newsItem->slug]));
                else
        ...
        CategoryTrans::where('active', 1)->where('slug', '!=', '')->get()->each(function (CategoryTrans $categoriesTrans) use ($siteMap, $lang, $pageUrls) {
            if (count($lang) > 1)
                $addUrl = Url::create(url($lang[$categoriesTrans->lang_id], [$pageUrls['catalog'][$categoriesTrans->lang_id], $categoriesTrans->slug]));
            else
        ...
        Product::leftJoin('product_trans', 'products.id', '=', 'product_trans.product_id')
            ->where('active', 1)->where('slug', '!=', '')->get()->each(function (Product $products) use ($siteMap, $lang, $pageUrls, $categoriesUrl) {
                if (count($lang) > 1)
                    $addUrl = Url::create(url($lang[$products->lang_id], [$pageUrls['catalog'][$products->lang_id], $categoriesUrl[$products->lang_id][$products->category_id], $products->slug]));
                else
                    $addUrl = Url::create(url($pageUrls['catalog'][$products->lang_id], $categoriesUrl[$products->lang_id][$products->category_id], $products->slug));
